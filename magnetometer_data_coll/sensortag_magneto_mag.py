...
def main():
    ...
    filename = "magnetometer_data_" + str(round(time.time())) + ".csv"
    f = open(filename,"a")
    ...
    ax.yaxis.grid(which="major",color='#C0C0C0', linestyle='-', linewidth=0.5)
    # line1, = ax.plot([i for i in range(SAMPLES)], mag_data, 'b-',linewidth=1)
    # line2, = ax.plot([i for i in range(SAMPLES)], pandas.rolling_mean(np.array(mag_data), ROLLING_WINDOW), 'r-',linewidth=3)
    # line3, = ax.plot([i for i in range(SAMPLES)], [sum(mag_data)/SAMPLES] * SAMPLES, 'g-', linestyle=':')
    line1, = ax.plot([], [], 'b-',linewidth=1, linestyle='-')
    line2, = ax.plot([], [], 'r-',linewidth=2)
    line3, = ax.plot([], [], 'g-', linestyle=':')
    line4, = ax.plot([], [], 'm-', linestyle=':')
    ...
    while(True):
	    ...
	    while True:
	       ...
	       if arg.magnetometer or arg.all:
	           try: 	
	           	magneto_mag = math.sqrt(tag.magnetometer.read()[0]**2+tag.magnetometer.read()[1]**2+tag.magnetometer.read()[2]**2)
	           	counter = counter + 1
	           except:
	           	print("EXCEPTION")
	           	sleep(2)
	           	break
	           print(str(time.time()) + "," + str(magneto_mag))
	           f.write(str(time.time()) + "," + str(magneto_mag) + "\n")
	           if(counter > IGNORE_FIRST_N_SAMPLES):
		           if(len(mag_data) == SAMPLES):
		             mag_data.pop(0)
		           mag_data.append(magneto_mag)
		           # print(mag_data)

		           if(len(mag_data) > 2):
			           plt.axis([0, len(mag_data), min(mag_data), max(mag_data)])
			           line1.set_ydata(mag_data)
			           line2.set_ydata(pandas.rolling_mean(np.array(mag_data), ROLLING_WINDOW))
			           line3.set_ydata([sum(mag_data)/len(mag_data)]*len(mag_data))

			           # print(len([sum(mag_data[0:floor(len(mag_data)/2)])/floor(len(mag_data)/2)]*floor(len(mag_data)/2) + [sum(mag_data[ceil(len(mag_data)/2):len(mag_data)])/ceil(len(mag_data)/2)]*ceil(len(mag_data)/2)))
			           line4.set_ydata([sum(mag_data[0:floor(len(mag_data)/2)])/floor(len(mag_data)/2)]*floor(len(mag_data)/2))
			           # +\
			           # [sum(mag_data[ceil(len(mag_data)/2):len(mag_data)])/(len(mag_data)-floor(len(mag_data)/2))]*ceil(len(mag_data)/2))

			           line1.set_xdata([i for i in range(1,len(mag_data) +1)])
			           line2.set_xdata([i for i in range(1,len(pandas.rolling_mean(np.array(mag_data), ROLLING_WINDOW))+1)])
			           line3.set_xdata([i for i in range(1,len(mag_data) + 1)])
			           line4.set_xdata([i for i in range(1,floor(len(mag_data)/2) + 1)])
			           # print([sum(mag_data)]*100)
			           fig.canvas.draw()
