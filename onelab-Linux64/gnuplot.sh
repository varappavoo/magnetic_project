...
# 2D VIEW
# set cbrange [0.00001:];
gnuplot -e "set cbtics format '%g';set xrange[-0.4:0.4];set yrange[-0.4:0.4];set zrange[-0.4:0.4];set hidden3d offset 0;set xlabel 'x';set ylabel 'y';set zlabel 'z';set view equal xyz; set palette defined (-1 'yellow',0 'red', 1 'black');;set palette maxcolors 25 ; set pm3d interpolate 0,0;plot  'b_box.dat.csv.4d.sphere.mag' every 1 using 1:2:(column(4)>0.000000001?(column(4)):1/0) pt 7 ps .5  palette" -p
gnuplot -e "set cbtics format '%g';set xrange[-0.4:0.4];set yrange[-0.4:0.4];set zrange[-0.4:0.4];set hidden3d offset 0;set xlabel 'x';set ylabel 'z';set zlabel 'z';set view equal xyz; set palette defined (-1 'yellow',0 'red', 1 'black');;set palette maxcolors 25 ; set pm3d interpolate 0,0;plot  'b_box.dat.csv.4d.sphere.mag' every 1 using 1:3:(column(4)>0.000000001?(column(4)):1/0) pt 7 ps .5  palette" -p
gnuplot -e "set cbtics format '%g';set xrange[-0.4:0.4];set yrange[-0.4:0.4];set zrange[-0.4:0.4];set hidden3d offset 0;set xlabel 'y';set ylabel 'z';                                   set palette defined (-1 'yellow',0 'red', 1 'black');;set palette maxcolors 25 ; set pm3d interpolate 0,0;plot  'b_box.dat.csv.4d.sphere.mag' every 1 using 2:3:(column(4)>0.000000001?(column(4)):1/0) pt 7 ps .5  palette" -p
