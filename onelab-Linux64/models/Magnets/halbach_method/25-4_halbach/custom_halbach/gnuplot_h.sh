...
gnuplot -e ";set cbtics format '%g';;set xrange[-0.4:0.4];set yrange[-0.4:0.4];set zrange[-0.4:0.4];set parametric;set hidden3d offset 0;set xlabel 'x';set ylabel 'y';set zlabel 'z';set view equal xyz;set term wxt; set palette defined (-1 'yellow',0 'red', 1 'black');;set palette maxcolors 25 ; set pm3d interpolate 0,0;splot  'b_box.dat.csv.4d.sphere.mag' every 10 using 1:2:3:(column(4)>0.0?(column(4)):1/0) pt 5 ps 1  palette" -p
