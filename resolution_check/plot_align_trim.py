...
def process_data(s1,s2, truncate_start, truncate_end, smooth_cut_off_freq=SMOOTH_CUT_OFF_FREQ):
	...
	# #######################################################
	# [s1_cA, s1_cD] = wavedec(s1_data, 'db1',level=1)
	# [s2_cA, s2_cD] = wavedec(s2_data, 'db1',level=1)

	# # t = s1_t[0:len(s1_cD)]

	# plt.plot(s1_cD, label='s1_cA')
	# plt.plot(s2_cD, label='s2_cA')
	...
	if(SHOW_PLOT):plt.plot(s1_t, s1_data,  label=args.s1_filename)
	# plt.plot(s1[:,0][mean_samples-1:], s1_data,  label='s1_avg')
	# plt.plot(s2[:,0], s2[:,2]-np.min(s2),  label='s2')
	if(SHOW_PLOT):plt.plot(s2_t, s2_data,  label=args.s2_filename)
	...
	if(SHOW_PLOT):plt.plot(xcorr_t, xcorr, label='xcorr')
	...
	if(COMPUTE_ANGULAR_FREQUENCY_DEGREE_DRIFT):
		...
		if(DEBUG > 0 and not CSV_OUTPUT):
			...
			t1.add_row([args.s1_filename, args.s2_filename, truncate_start, truncate_end, SMOOTH_CUT_OFF_FREQ, time_shift/1000, round((2*math.pi)/(1/angular_freq_s1),5),round((2*math.pi)/(1/angular_freq_s2),5), colored(round(degree_shift,5), "cyan")])

			if(min_time_shift == None or min_time_shift > math.fabs(time_shift/1000)):
				min_time_shift = math.fabs(time_shift/1000)
				min_time_shift_row = [args.s1_filename, args.s2_filename, truncate_start, truncate_end, SMOOTH_CUT_OFF_FREQ, colored(time_shift/1000, "red"), round((2*math.pi)/(1/angular_freq_s1),5),round((2*math.pi)/(1/angular_freq_s2),5), colored(round(degree_shift,5), "red")]

			print( "\n".join(t1.get_string().splitlines()[-2:]) )

		elif(CSV_OUTPUT):
			print(args.s1_filename, ",", args.s2_filename, ",", truncate_start, ",", truncate_end, ",", SMOOTH_CUT_OFF_FREQ,",", time_shift/1000, "," ,  round((2*math.pi)/(1/angular_freq_s1),5), ",",round((2*math.pi)/(1/angular_freq_s2),5), ",", round(angular_freq_s1,5), ",",  round(angular_freq_s2,5), ",", degree_shift%360)
...
t1 = PrettyTable(["file s1                     ", "file s2                     ", "tstart ", "tend   ","fs_C_off","time_shift (s)", "period s1", "period s2", "degree_shift %360"])
